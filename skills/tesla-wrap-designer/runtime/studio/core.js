...
export function upgradeProject(p) {
  if (p.layers.some((l) => l.type === "stroke")) {
    const strokes = p.layers.filter((l) => l.type === "stroke");
    p.layers = p.layers.filter((l) => l.type !== "stroke");
    p.layers.push({
      id: "legacy-paint-" + crypto.randomUUID(),
      type: "paint",
...
export function validateProject(p, ids) {
  ...
  const paint = p.layers.filter((l) => l.type === "paint");
  if (
    paint.some(
      (l) =>
        !Array.isArray(l.strokes) ||
        l.strokes.some((s) => !s || s.type !== "stroke"),
    )
  )
    throw Error("Invalid paint layer.");
  const strokes = paint.flatMap((l) => l.strokes);
  if (
    strokes.length + p.layers.filter((l) => l.type === "stroke").length >
    10000
  )
    throw Error("Maximum 10,000 drawing strokes per project.");
  for (const l of [...p.layers, ...strokes]) {
    if (!["image", "text", "stroke", "paint"].includes(l.type))
    ...
    if (
      l.type === "paint" &&
      (l.x !== 0 || l.y !== 0 || l.rotation !== 0 || l.scale !== 1 || l.flip)
    )
      throw Error("Paint layers use canvas coordinates.");
    if (
      l.type === "image" &&
